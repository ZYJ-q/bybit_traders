...
async fn real_time(
    ...
    loop {
        ...
        let last_time = time_min - 1000*60*60*24 * minut_end;

        let mut end_times = 0;
        
        if time_minut_id == 2 {
            end_times = time_min - 1000*60*60*24 * minut_end;
        } else {
            end_times = last_time + 1000* (time_minut_id -2) * 60
        }
        let last_time_min = end_times + 1000*2*60 ;
        ...
        // 等待下次执行
        info!("waiting for next real time task...({})", 200 * 10);
        tokio::time::delay_for(Duration::from_millis(200 * 10)).await;
    }
}
